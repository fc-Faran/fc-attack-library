...
function Invoke-AttackLibRunner {
    [CmdletBinding(
    ...
    Process {
        ...
        function Invoke-AttackLibTestFromScheduleRow ($tr) {
            ...
            if (($sc -eq 'public') -or ($null -eq $sc)) {
                Invoke-AttackLibTest $tr.Technique -TestGuids $tr.auto_generated_guid -InputArgs $tr.InputArgs -TimeoutSeconds $tr.TimeoutSeconds -ExecutionLogPath $artConfig.execLogPath -PathToAtomicsFolder $artConfig.PathToPublicAtomicsFolder @htvars -supressPathToAtomicsFolder
            }
            elseif ($sc -eq 'private') {
                Invoke-AttackLibTest $tr.Technique -TestGuids $tr.auto_generated_guid -InputArgs $tr.InputArgs -TimeoutSeconds $tr.TimeoutSeconds -ExecutionLogPath $artConfig.execLogPath -PathToAtomicsFolder $artConfig.PathToPrivateAtomicsFolder @htvars -supressPathToAtomicsFolder
            }
            if ($timeToPause -gt 0) {
        ...
        # Convert OtherArgs to hashtable so we can pass it through to the call to Invoke-AttackLibTest
        $htvars = @{}
        if ($OtherArgs) {
        ...
        if ($Cleanup -or $ShowDetails -or $CheckPrereqs -or $ShowDetailsBrief -or $GetPrereqs -or $listOfAtomics) {
            $schedule | ForEach-Object {
                Invoke-AttackLibTestFromScheduleRow $_
            }
            return
        }
        ...
        if ([string]::IsNullOrWhiteSpace($guid)) {
            ...
            if ($null -ne $tr) {
                if ($scheduledTaskCleanup) {
                    ...
                    Invoke-AttackLibTestFromScheduleRow $tr
                }
                else {
