...
function New-AttackLibTechnique {
    <#
.SYNOPSIS
...
.PARAMETER AttackLibTests

Specifies one or more atomic tests. Atomic tests are created using the New-AttackLibTest function.

.EXAMPLE

$InputArg1 = New-AttackLibTestInputArgument -Name filename -Description 'location of the payload' -Type Path -Default 'PathToAtomicsFolder\T1118\src\T1118.dll'
$InputArg2 = New-AttackLibTestInputArgument -Name source -Description 'location of the source code to compile' -Type Path -Default 'PathToAtomicsFolder\T1118\src\T1118.cs'

$AttackLibTest1 = New-AttackLibTest -Name 'InstallUtil uninstall method call' -Description 'Executes the Uninstall Method' -SupportedPlatforms Windows -InputArguments @($InputArg1, $InputArg2) -ExecutorType CommandPrompt -ExecutorCommand @'
...
$AttackLibTest2 = New-AttackLibTest -Name 'InstallUtil GetHelp method call' -Description 'Executes the Help property' -SupportedPlatforms Windows -InputArguments @($InputArg1, $InputArg2) -ExecutorType CommandPrompt -ExecutorCommand @'
C:\Windows\Microsoft.NET\Framework\v4.0.30319\csc.exe /target:library /out:#{filename} #{source}
C:\Windows\Microsoft.NET\Framework\v4.0.30319\InstallUtil.exe /? #{filename}
'@

$AttackLibTechnique = New-AttackLibTechnique -AttackTechnique T1118 -DisplayName InstallUtil -AttackLibTests $AttackLibTest1, $AttackLibTest2

# Everything is ready to convert to YAML now!
$AttackLibTechnique | ConvertTo-Yaml | Out-File T1118.yaml
...
The output of New-AttackLibTechnique is designed to be piped to ConvertTo-Yaml.
#>
    ...
    $AttackLibTechniqueInstance = [AttackLibTechnique]::new()
    ...
    $AttackLibTechniqueInstance.attack_technique = $AttackTechnique
    $AttackLibTechniqueInstance.display_name = $DisplayName
    $AttackLibTechniqueInstance.atomic_tests = $AttackLibTests

    return $AttackLibTechniqueInstance
}

function New-AttackLibTest {
    <#
.SYNOPSIS
...
Specifies one or more input arguments. Input arguments are defined using the New-AttackLibTestInputArgument function.
...
Specifies one or more dependencies. Dependencies are defined using the New-AttackLibTestDependency function.
...
$AttackLibTest = New-AttackLibTest -Name 'InstallUtil uninstall method call' -Description 'Executes the Uninstall Method' -SupportedPlatforms Windows -InputArguments $InputArg1, $InputArg2 -ExecutorType CommandPrompt -ExecutorCommand @'
C:\Windows\Microsoft.NET\Framework\v4.0.30319\csc.exe /target:library /out:#{filename}  #{source}
C:\Windows\Microsoft.NET\Framework\v4.0.30319\InstallUtil.exe /logfile= /LogToConsole=false /U #{filename}
'@
...
Outputs an object representing an atomic test. This object is intended to be supplied to the New-AttackLibTechnique -AttackLibTests parameter.

The output of New-AttackLibTest can be piped to ConvertTo-Yaml. The resulting output can be added to an existing atomic technique YAML doc.
#>

    [CmdletBinding(DefaultParameterSetName = 'AutomatedExecutor')]
    [OutputType([AttackLibTest])]
    param (
        [Parameter(Mandatory)]
        [String]
        [ValidateNotNullOrEmpty()]
    ...
    $AttackLibTestInstance.name = $Name
    $AttackLibTestInstance.description = $Description
    $AttackLibTestInstance.supported_platforms = $SupportedPlatforms | ForEach-Object { $_.ToLower() }
    ...
    if ($DependencyExecutorType) {
        switch ($DependencyExecutorType) {
            'CommandPrompt' { $AttackLibTestInstance.dependency_executor_name = 'command_prompt' }
            default { $AttackLibTestInstance.dependency_executor_name = $DependencyExecutorType.ToLower() }
        }
    ...
    if ($InputArguments.Count) {
        ...
        foreach ($InputArg in $InputArguments) {
            ...
            $NewInputArg = [AttackLibInputArgument]::new()
        ...
        $AttackLibTestInstance.input_arguments = $InputArgHashtable
    }
    ...
    return $AttackLibTestInstance
}

function New-AttackLibTestDependency {
    <#
.SYNOPSIS
...
$Dependency = New-AttackLibTestDependency -Description 'Folder to zip must exist (#{input_file_folder})' -PrereqCommand 'test -e #{input_file_folder}' -GetPrereqCommand 'echo Please set input_file_folder argument to a folder that exists'

.OUTPUTS

AttackLibDependency

Outputs an object representing an atomic test dependency. This object is intended to be supplied to the New-AttackLibTest -Dependencies parameter.
...
#>
    ...
    [CmdletBinding()]
    [OutputType([AttackLibDependency])]
    param (
        [Parameter(Mandatory)]
        [String]
        [ValidateNotNullOrEmpty()]
...
function New-AttackLibTestInputArgument {
    <#
.SYNOPSIS
...
Specifies a default value for an input argument if one is not specified via the Invoke-AttackLibTest -InputArgs parameter.

.EXAMPLE

$AttackLibInputArgument = New-AttackLibTestInputArgument -Name 'rar_exe' -Type Path -Description 'The RAR executable from Winrar' -Default '%programfiles%\WinRAR\Rar.exe'

.OUTPUTS

AttackLibInputArgument

Outputs an object representing an atomic test input argument. This object is intended to be supplied to the New-AttackLibTest -InputArguments parameter.
#>

    [CmdletBinding(DefaultParameterSetName = 'PredefinedType')]
    [OutputType([AttackLibInputArgument])]
    param (
        [Parameter(Mandatory)]
        [String]
        [ValidateNotNullOrEmpty()]
    ...
    $AtomicInputArgInstance = [AttackLibInputArgument]::new()
    ...
    # Add Name as a note property since the Name property cannot be defined in the AttackLibInputArgument
