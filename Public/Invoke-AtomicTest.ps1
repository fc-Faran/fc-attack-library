function Invoke-AttackLibTest {
    ...
    Param(
        ...
        [Parameter(Mandatory = $false,
            ParameterSetName = 'technique')]
        [String]
        $ExecutionLogPath = $( if ($IsLinux -or $IsMacOS) { "/tmp/Invoke-AttackLibTest-ExecutionLog.csv" } else { "$env:TEMP\Invoke-AttackLibTest-ExecutionLog.csv" }),
    ...
    PROCESS {
        ...
        $PathToPayloads = if ($Session) { "$tmpDir`AttackLib" }  else { $PathToAtomicsFolder }

        # Since there might a comma(T1559-1,2,3) Powershell takes it as array.
        # So converting it back to string.
        if ($AttackLibTechnique -is [array]) {
            $AttackLibTechnique = $AttackLibTechnique -join ","
        }

        # Splitting Atomic Technique short form into technique and test numbers.
        $AttackLibTechniqueParams = ($AttackLibTechnique -split '-')
        $AttackLibTechnique = $AttackLibTechniqueParams[0]

        if ($AttackLibTechniqueParams.Length -gt 1) {
            $ShortTestNumbers = $AttackLibTechniqueParams[-1]
        }
        ...
        if ($isLoggingModuleSet) {
            ConvertTo-LoggerArray $LoggingModule | ForEach-Object {
                ...
                if ($_ -eq "Attire-ExecutionLogger") { $ExecutionLogPath = $ExecutionLogPath.Replace("Invoke-AttackLibTest-ExecutionLog.csv", "Invoke-AttackLibTest-ExecutionLog-timestamp.json") }
            ...
            $commandLine = "Invoke-AttackLibTest $AttackLibTechnique"
        ...
        function Invoke-AttackLibTestSingle ($AT) {

            $AT = $AT.ToUpper()
            $pathToYaml = Join-Path $PathToAtomicsFolder "\$AT\$AT.yaml"
            if (Test-Path -Path $pathToYaml) { $AttackLibTechniqueHash = Get-AttackLibTechnique -Path $pathToYaml }
            else {
            ...
            $techniqueString = ""
            foreach ($technique in $AttackLibTechniqueHash) {
                ...
                $props = @{
                    ...
                    Status          = 'Progress:'
                    PercentComplete = ($techniqueCount / ($AttackLibTechniqueHash).Count * 100)
                }
        ...
        } # End of Invoke-AttackLibTestSingle function

        if ($AttackLibTechnique -eq "All") {
            function Invoke-AllTests() {
                $AllAttackLibTests = New-Object System.Collections.ArrayList
                Get-ChildItem $PathToAtomicsFolder -Directory -Filter T* | ForEach-Object {
                    ...
                    if ( $currentTechnique -match "T[0-9]{4}.?([0-9]{3})?" ) { $AllAttackLibTests.Add($currentTechnique) | Out-Null }
                }
                $AllAttackLibTests.GetEnumerator() | Foreach-Object { Invoke-AttackLibTestSingle $_ }
            }
            ...
            Invoke-AttackLibTestSingle $AttackLibTechnique
        }
