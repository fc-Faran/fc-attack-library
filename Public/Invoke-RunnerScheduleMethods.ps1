...
function Loop($fileList, $atomicType) {
    ...
    $fileList | ForEach-Object {
        ...
        if ( $currentTechnique -ne "index" ) {
            $technique = Get-AttackLibTechnique -Path $_.FullName
            if ($technique) {
                $technique.atomic_tests | ForEach-Object -Process {
                    ...
                    $dummy = $AllAttackLibTests.Add(($test))
                }
            }
        }
    }
    return $AllAttackLibTests
...
function Get-NewSchedule() {
    ...
    $AllAttackLibTests = New-Object System.Collections.ArrayList
    try { $AllAttackLibTests.AddRange($publicAtomics) }catch {}
    try { $AllAttackLibTests.AddRange($privateAtomics) }catch {}
    return $AllAttackLibTests
}

function Get-ScheduleRefresh() {
    $AllAttackLibTests = Get-NewSchedule
    ...
    $update = $false
    foreach ($guid in $AllAttackLibTests | Select-Object -ExpandProperty auto_generated_guid) {
        $fresh = $AllAttackLibTests | Where-Object { $_.auto_generated_guid -eq $guid }
