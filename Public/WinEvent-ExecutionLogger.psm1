function Start-ExecutionLog($startTime, $logPath, $targetHostname, $targetUser, $commandLine, $isWindows) {
    if ($isWindows -and -not [System.Diagnostics.EventLog]::Exists('Attack Lib')) {
        New-EventLog -Source "Applications and Services Logs" -LogName "Attack Lib"
    }
}
...
function Write-ExecutionLog($startTime, $stopTime, $technique, $testNum, $testName, $testGuid, $testExecutor, $testDescription, $command, $logPath, $targetHostname, $targetUser, $res, $isWindows) {
    ...
    Write-EventLog  -Source "Applications and Services Logs" -LogName "Attack Lib" -EventID 3001 -EntryType Information -Message $msg -Category 1 -RawData 10, 20
}
