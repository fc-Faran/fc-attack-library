...
$artConfig = [PSCustomObject]@{
  ...
  # [optional(if using default install paths)] Paths to your Attack Lib "atomics" folder and your "invoke-attacklib" folder
  PathToInvokeFolder         = Join-Path $( if ($IsLinux -or $IsMacOS) { "~" } else { "C:" })  "/AttackLib/invoke-attacklib" # this is the default install path so you probably don't need to change this
  PathToPublicAtomicsFolder  = Join-Path $( if ($IsLinux -or $IsMacOS) { "~" } else { "C:" })  "AttackLib/atomics" # this is the default install path so you probably don't need to change this
  ...
  kickOffDelay               = New-TimeSpan -Minutes 0 # an additional delay before Invoke-KickoffAtomicRunner calls Invoke-AttackLibRunner
...
# If you create a file called privateConfig.ps1 in the same directory as you installed Invoke-AttackLib you can overwrite any of these settings with your custom values
