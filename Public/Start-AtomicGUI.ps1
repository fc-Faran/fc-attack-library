    ...
    $BuildAndDisplayYamlScriptBlock = {
        ...
        $InputArgCards.GetEnumerator() | ForEach-Object {
            if ($_.Value -eq $false) {
                ...
                $NewInputArg = New-AttackLibTestInputArgument -Name $InputArgName -Description $InputArgDescription -Type $InputArgType -Default $InputArgDefault -WarningVariable +warnings
        ...
        $depCards.GetEnumerator() | ForEach-Object {
            if ($_.Value -eq $false) {
                ...
                $NewDep = New-AttackLibTestDependency -Description $depDescription -PrereqCommand $prereqCommand -GetPrereqCommand $getPrereqCommand -WarningVariable +warnings
                $dependencies += $NewDep
            }
        }
        $depParams = @{ }
        ...
        $AttackLibTest = New-AttackLibTest -Name $testName -Description $testDesc -SupportedPlatforms $platforms -InputArguments $inputArgs -ExecutorType $executor -ExecutorCommand $attackCommands -WarningVariable +warnings @depParams
        $yaml = ($AttackLibTest | ConvertTo-Yaml) -replace "^", "- " -replace "`n", "`n  "
