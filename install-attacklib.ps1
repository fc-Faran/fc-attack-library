...
function Install-AttackLib {

    <#
    .SYNOPSIS

        This is a simple script to download and install the Attack Lib Invoke-AttackLib Powershell Framework.

        Atomic Function: Install-AttackLib
    ...
    .PARAMETER DownloadPath
        ...
        Specifies the desired path to download Attack Lib.

    .PARAMETER InstallPath

        Specifies the desired path for where to install Attack Lib.
...
#>
    ...
    Param(
        ...
        [string]$InstallPath = $( if ($IsLinux -or $IsMacOS) { $Env:HOME + "/AttackLib" } else { $env:HOMEDRIVE + "\AttackLib" }),
    ...
    Try {
        ...
        $InstallPathwIart = Join-Path $InstallPath "invoke-attacklib"
        $modulePath = Join-Path "$InstallPath" "invoke-attacklib\Invoke-AttackLib.psd1"
        if ($Force -or -Not (Test-Path -Path $InstallPathwIart )) {
            ...
            $url = "https://github.com/$RepoOwner/invoke-attacklib/archive/$Branch.zip"
            ...
            $iartFolderUnzipped = Join-Path $zipDest "invoke-attacklib-$Branch"
            ...
            if ($getAtomics) {
                ...
                Invoke-Expression (New-Object Net.WebClient).DownloadString("https://raw.githubusercontent.com/$RepoOwner/invoke-attacklib/$Branch/install-atomicsfolder.ps1"); Install-AtomicsFolder -InstallPath $InstallPath -DownloadPath $DownloadPath -Force:$Force -RepoOwner $RepoOwner -NoPayloads:$NoPayloads
            }

            Write-Host "Installation of Invoke-AttackLib is complete. You can now use the Invoke-AttackLibTest function" -Fore Yellow
            Write-Host "See Wiki at https://github.com/$repoOwner/invoke-attacklib/wiki for complete details" -Fore Yellow
        }
        else {
    ...
    Catch {
        Write-Error "Installation of AttackLib Failed."
